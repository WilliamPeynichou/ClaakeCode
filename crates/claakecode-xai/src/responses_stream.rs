                ...
                match tokio::time::timeout(STREAM_IDLE_TIMEOUT, source.next()).await {
                    ...
                    Ok(None) => {
                        if !saw_any_event {
                            return Some((
                                Err(AppError::RetryableStream {
                                    message: format!(
                                        "xai {source_name} closed before any event; the server likely dropped the connection"
                                    ),
                                    delay_ms: None,
                                }),
                                (source, parser, pending, true, saw_any_event),
                        ...
                        return Some((
                            Err(AppError::RetryableStream {
                                message: format!(
                                    "xai {source_name} stream closed before response.completed"
                                ),
                                delay_ms: None,
                            }),
                            (source, parser, pending, true, saw_any_event),
                    ...
                    Err(_) => {
                        return Some((
                            Err(AppError::RetryableStream {
                                message: format!("idle timeout waiting for xAI {source_name}"),
                                delay_ms: None,
                            }),
                            (source, parser, pending, true, saw_any_event),
                        ));
    ...
    E: std::error::Error + Send + Sync + 'static,
{
    body.eventsource()
        .map_err(|err| AppError::RetryableStream {
            message: format!("xai SSE error: {err}"),
            delay_ms: None,
        })
        .try_filter_map(|event| async move {
            ...
            serde_json::from_str::<Value>(&event.data)
                .map(Some)
                .map_err(|err| AppError::Decode(format!("bad xai SSE event: {err}")))
        })
}
