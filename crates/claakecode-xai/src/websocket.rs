...
impl ResponsesWebsocketConnection {
    async fn connect(config: &XaiConfig, bearer: &BearerToken) -> Result<Self> {
        let ws_url = responses_websocket_url(&config.composer_base_url)?;
        let mut request = ws_url.as_str().into_client_request().map_err(|err| {
            AppError::Stream(format!("failed to build xAI websocket request: {err}"))
        })?;
        ...
        if let Some(account_id) = bearer.account_id.as_deref() {
            headers.insert(
                "xai-account-id",
                header_value(account_id, "xai-account-id")?,
            );
        }

        tracing::debug!(url = %ws_url, "connecting xAI responses websocket");
        ...
        tracing::debug!(status = %response.status(), "xAI responses websocket connected");
...
pub(crate) fn responses_websocket_url(base_url: &str) -> Result<Url> {
    let mut url = Url::parse(base_url)
        .map_err(|err| AppError::InvalidRequest(format!("invalid xAI base URL: {err}")))?;
    match url.scheme() {
        "http" => {
        ...
        scheme => {
            return Err(AppError::InvalidRequest(format!(
                "unsupported xAI websocket URL scheme: {scheme}"
            )));
        }
    }
...
async fn run_websocket_response_stream(
    ...
    let object = envelope.as_object_mut().ok_or_else(|| {
        AppError::Decode("xAI responses request did not serialize to an object".into())
    })?;
    object.insert(
        "type".to_string(),
    ...
    let payload = serde_json::to_string(&envelope).map_err(|err| {
        AppError::Decode(format!("failed to encode xAI websocket request: {err}"))
    })?;
    ...
    let result = match guard.as_mut() {
        ...
        None => Err(AppError::RetryableStream {
            message: "xAI websocket connection is closed".to_string(),
            delay_ms: None,
        }),
    };
    if result.is_err() {
...
async fn run_websocket_response_stream_locked(
    ...
    .map_err(|_| AppError::RetryableStream {
        message: "idle timeout sending xAI websocket request".to_string(),
        delay_ms: None,
    })?
    .map_err(|err| AppError::RetryableStream {
        message: format!("failed to send xAI websocket request: {err}"),
        delay_ms: None,
    })?;
    ...
    loop {
        let message = tokio::time::timeout(STREAM_IDLE_TIMEOUT, ws_stream.next())
            .await
            .map_err(|_| AppError::RetryableStream {
                message: "idle timeout waiting for xAI websocket".to_string(),
                delay_ms: None,
            })?;
        let message = match message {
            ...
            Some(Err(err)) => {
                return Err(AppError::RetryableStream {
                    message: format!("failed to read xAI websocket event: {err}"),
                    delay_ms: None,
                });
            }
            None => {
                return Err(AppError::RetryableStream {
                    message: "xAI websocket closed before response.completed".to_string(),
                    delay_ms: None,
                });
            }
        };
        ...
        match message {
            Message::Text(text) => {
                ...
                if tx_event.send(Ok(event)).await.is_err() {
                    return Err(AppError::Stream(
                        "xAI websocket event consumer dropped".to_string(),
                    ));
                }
                if terminal {
                    return Ok(());
            ...
            Message::Binary(_) => {
                return Err(AppError::RetryableStream {
                    message: "unexpected binary xAI websocket event".to_string(),
                    delay_ms: None,
                });
            }
            Message::Close(frame) => {
                return Err(AppError::RetryableStream {
                    message: format!(
                        "xAI websocket closed before response.completed: code={:?} reason={:?}",
...
fn parse_text_event(text: &str) -> Result<Value> {
    tracing::trace!(target: "claakecode_xai::websocket::wire", "xAI websocket event: {text}");
    ...
    serde_json::from_str::<Value>(text)
        .map_err(|err| AppError::Decode(format!("bad xAI websocket event: {err}")))
}
...
fn map_ws_connect_error(err: WsError, url: &Url) -> AppError {
    match err {
        ...
        WsError::ConnectionClosed | WsError::AlreadyClosed => {
            retryable_ws_error("xAI websocket closed during connect".into())
        }
        WsError::Io(err) => AppError::Network(format!("xAI websocket network error: {err}")),
        other => retryable_ws_error(format!("failed to connect xAI websocket: {other}")),
    }
}
...
mod tests {
    ...
    fn websocket_url_appends_responses_and_switches_scheme() {
        let url = responses_websocket_url("https://api.xai.com/v1").unwrap();
        assert_eq!(url.as_str(), "wss://api.xai.com/v1/responses");
    }
