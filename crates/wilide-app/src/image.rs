...
use wilide_openai::{Credential, MODEL_ID as OPENAI_RESPONSES_IMAGE_MODEL};
...
const USER_AGENT: &str = "wilide/0.1";
...
impl CreateImageTool {
    ...
    fn save_image(&self, bytes: &[u8], extension: &str, idx: usize) -> Result<String> {
        let dir = self.workspace_root.join(".wilide/images");
        fs::create_dir_all(&dir).context("unable to create .wilide/images")?;
        let name = format!("{}-{}.{}", now_ms(), idx + 1, extension);
        let relative_path = format!(".wilide/images/{name}");
