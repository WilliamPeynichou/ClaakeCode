...
fn powershell_script(command: &str) -> String {
    format!(
        r#"$__wilideUtf8 = [System.Text.UTF8Encoding]::new($false)
[Console]::InputEncoding = $__wilideUtf8
[Console]::OutputEncoding = $__wilideUtf8
$OutputEncoding = $__wilideUtf8
...
$__wilideSuccess = $?
$__wilideExitCode = if ($global:LASTEXITCODE -is [int]) {{ $global:LASTEXITCODE }} elseif ($__wilideSuccess) {{ 0 }} else {{ 1 }}
exit $__wilideExitCode
"#
    )
}
...
mod tests {
    ...
    fn temp_workspace(name: &str) -> PathBuf {
        let root =
            std::env::temp_dir().join(format!("wilide-bash-test-{name}-{}", std::process::id()));
        let _ = fs::remove_dir_all(&root);
